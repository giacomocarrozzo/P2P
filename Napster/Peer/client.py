...
class PeerClient(object):
	...
	def login(self):
		...
		message_type = self.dir_socket.recv(4)
		session_id = self.dir_socket.recv(16)
	...
	def logout(self):
		if (self.app.context['sessionid']):
			...
			message_type = self.dir_socket.recv(4)
			file_deleted = self.dir_socket.recv(3)
	...
	def addFile(self, filename, md5):
		if self.app.context["sessionid"]:
			...
			message_type = self.dir_socket.recv(4)
			copy_numbers = self.dir_socket.recv(3)
	...
	def searchFile(self):
		...
		if not len(searchString) == 0:
			...
			if self.app.context["sessionid"]:
				...
				message_type = self.dir_socket.recv(4)
				num = int(self.dir_socket.recv(3))
				...
				if num != 0:
					...
					for f in range(num):
						...
						file_md5 = self.dir_socket.recv(32)
						nome = self.dir_socket.recv(100)
						copie = int(self.dir_socket.recv(3))
						...
						for c in range(copie):
							...
							ip = self.dir_socket.recv(55)
							port = int(self.dir_socket.recv(5))
